...
def compute_error(prd,y):
    ...
    a=np.multiply((prd-y),(prd-y))
    a=np.multiply(a,a)
    return  (float(np.sum(a))/(4*y.shape[0]))
...
def back_prop(xtrain,weights1,bias1,weights2,bias2,ytrain,alpha,yval,xval):
    ...
    for i in range(0,epochs):
        ...
        multi=np.multiply(np.multiply(prd-ytrain,prd-ytrain),prd-ytrain)
        delta2=np.multiply(np.multiply(multi,prd),1- prd )
